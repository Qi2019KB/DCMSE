...
class G_Conv(nn.Module):
    def __init__(self, inp_dim, out_dim, kernel_size=3, stride=1, grp=2, bn=False, relu=True):
        super(G_Conv, self).__init__()
        self.inp_dim = inp_dim
        self.conv = nn.Conv2d(inp_dim, out_dim, kernel_size, stride, padding=(kernel_size - 1) // 2, bias=True, groups=grp)
        self.relu = None
        self.bn = None
        if relu:
            self.relu = nn.ReLU()
