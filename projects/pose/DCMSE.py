...
def main(args):
    ...
    maxBS = args.trainBS if args.trainBS > args.inferBS else args.inferBS
    model = models.__dict__["HG_ems"](maxBS, args.kpsCount, args.nStack, args.nStream, splitFactor=args.splitFactor, switchRandom=args.switchRandom).to(args.device)
    optim = TorchAdam(model.parameters(), lr=args.lr, weight_decay=args.wd)
    hg_pNum = sum(p.numel() for p in model.parameters())
    logc = "=> initialized HG_ems models (nStack: {}, params: {})".format(args.nStack, format(hg_pNum / 1000000.0, ".2f"))
    logger.print("L1", logc, start=loadingTM)
    # endregion

    # region 3. iteration
    logger.print("L1", "=> start, Hourglass_ems with semi-supervised learning")
...
def train(trainLoader, model, optim, args):
    ...
    meanCons_lossFunc = GateJointDistLoss(useSampleWeight=True).to(args.device)
    stackCons_lossFunc = GateJointDistLoss().to(args.device)
    model.train()
    for bat, (imgMap, kpsHeatmap, meta) in enumerate(trainLoader):
        ...
        outs_ms, features_ms, features_ms_f = model(imgMap)
        # endregion
        ...
        if args.meanConsWeight > 0:
            ...
            for sIdx in range(args.nStack):
                for stIdx in range(args.nStream):
                    loss, n = meanCons_lossFunc(outs_ms[:, sIdx, stIdx], outs_ms_mean[:, sIdx], sampleWeight=sampleWeight_negative)
                    mcc_sum += loss
                    mcc_count += n
        ...
        if args.stackConsWeight > 0:
            ...
            scc_sum, scc_count = stackCons_lossFunc(outs_ms[:, 0], outs_b)
...
def initArgs(params=None):
    ...
    parser.add_argument("--switchRandom", default="False")
    parser.add_argument("--splitFactor", default=0.2, type=float)
    ...
    parser.add_argument("--meanConsWeight_max", default=50.0, type=float)
    parser.add_argument("--meanConsWeight_rampup", default=50, type=int)
    parser.add_argument("--stackConsWeight_max", default=5.0, type=float)
